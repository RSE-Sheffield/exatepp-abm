 ...
 * @param exposedBy id of the source of exposure
 * @param sourceAge age of the source individual
 * @param sourceTimeExposed the time at which the source of infection was exposed themselves, for transmission file output
 * @param transmissionEventNetwork integer representing which network was the source of the transmission. 0 = household, 1 = work, 2 = random. Should be an enum @todo
 */
template<typename MsgIn, typename MsgOut>
FLAMEGPU_DEVICE_FUNCTION void susceptibleToExposed(flamegpu::DeviceAPI<MsgIn, MsgOut>* FLAMEGPU, disease::SEIR::InfectionStateUnderlyingType& infectionStatus, const flamegpu::id_t exposedBy, const std::uint32_t sourceTimeExposed, const std::uint8_t transmissionEventNetwork) {
    ...
    // Update agent variables tracking the source of exposure, time of exposure of the source and network of exposure.
    // These are used for timeSerious outputs, but difficult to isolate/abstract so always tracked.
    FLAMEGPU->template setVariable<std::uint8_t>(person::v::TF_EVENT_NETWORK, transmissionEventNetwork);
    FLAMEGPU->template setVariable<std::uint32_t>(person::v::TF_SOURCE_ID, exposedBy);
    FLAMEGPU->template setVariable<std::uint32_t>(person::v::TF_SOURCE_TIME_EXPOSED, sourceTimeExposed);
}
 ...
 *
 * @param FLAMEGPU flamegpu device api object
 * @param infectionStatus current infection status for the individual to b
    FLAMEGPU->template setVariable<disease::SEIR::InfectionStateUnderlyingType>e mutated in-place
