...
class TransmissionFile : public OutputFile {
 public:
    ...
    /**
     * Fix data in entries for data about the source agent, via their ID.
     *
     * @param ageDemographics age for each agent, indexed  by their id-1
     * @param householdIndices household index for each agent, indexed  by their id-1
     * @param workplaceIndices workplace index for each agent, indexed  by their id-1
     */
    void fixSourceAgentData(std::vector<demographics::AgeUnderlyingType> ageDemographics, std::vector<std::uint32_t> householdIndices, std::vector<workplace::WorkplaceUnderlyingType> workplaceIndices);
